...
class ImageLogger(Callback):
    ...
    def log_local(self, save_dir, split, images,
        ...
        for k in images:
            # Transformation pour sauvegarde disque (0-255)
            # Attention : cela modifie le tenseur en place, donc on fait ça en dernier !
            img_to_save = (images[k] + 1.0) * 127.5 
            img_to_save = torch.clamp(img_to_save, 0, 255)
            
            grid = torchvision.utils.make_grid(img_to_save, nrow=4)
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        if (self.check_frequency(batch_idx) and 
            ...
            if hasattr(pl_module.logger, 'experiment'):
                for k in images:
                    # Normalisation [-1, 1] -> [0, 1] pour TensorBoard
                    grid_tb = (images[k] + 1.0) / 2.0
                    grid_tb = torchvision.utils.make_grid(grid_tb, nrow=4)
                    
                    pl_module.logger.experiment.add_image(f'{split}/{k}', grid_tb, pl_module.global_step)
