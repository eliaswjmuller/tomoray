"""
CPU-only version of generate_all.py for local testing on MPS/non-CUDA machines.
Identical logic except tigre.Ax is replaced with a CPU parallel-beam ray-sum.

Usage:
    python data/generate_local_cpu.py \
        --inputDir "../_local_data/TomoRay CTSpine1k" \
        --configPath config/dataset/config_ctspine1k.yaml \
        --outputFolder ./data/ctspine1k_pickles \
        --orientation RAS \
        --fix_reflection auto
"""
...
def cpu_forward_project(vol_zyx: np.ndarray, geo_data: dict, angles: np.ndarray):
    """
    CPU replacement for tigre.Ax().
    Parallel-beam forward projection: rotate volume around Y-axis (TIGRE convention)
    and sum along the ray direction.

    TIGRE parallel-beam convention:
    - Volume is (Z, Y, X)
    - Rotation is around Y-axis
    - Detector plane is (V=Z, U=X)
    - Rays travel along the rotated X direction

    Returns: projections of shape (N_angles, nDetector[0], nDetector[1])
    """
    nZ, nY, nX = vol_zyx.shape
    n_det_v, n_det_u = geo_data["nDetector"]

    # Voxel size in meters (ZYX)
    dVoxel = np.array(geo_data["dVoxel"][::-1]) / 1000.0  # ZYX

    projections = np.zeros((len(angles), n_det_v, n_det_u), dtype=np.float32)

    for i, angle in enumerate(angles):
        angle_deg = np.degrees(float(angle))
        # TIGRE rotates around Y-axis in ZYX space (axis index 1)
        # scipy.ndimage.rotate: axes=(2, 0) rotates in the XZ plane = rotation around Y
        rotated = scipy.ndimage.rotate(vol_zyx, angle_deg, axes=(2, 0),
                                       reshape=False, order=1, mode='constant', cval=0.0)

        # Sum along Y-axis (axis 1) — ray direction after rotation
        # Result shape: (Z, X) = (nZ, nX)
        proj = np.sum(rotated, axis=1) * dVoxel[1]  # multiply by voxel thickness along ray

        # Resize to detector dimensions if needed
        if proj.shape[0] != n_det_v or proj.shape[1] != n_det_u:
            zoom_factors = (n_det_v / proj.shape[0], n_det_u / proj.shape[1])
            proj = scipy.ndimage.zoom(proj, zoom_factors, order=1)
...
def generator_one(matPath: str, config_data: dict, outputPath: str,
    ...
    projections = cpu_forward_project(vol_tigre.astype(np.float32), data, angles)
